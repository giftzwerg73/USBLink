...
static bool make_mean(uint32_t *reslut, uint32_t newval, uint8_t mean_nr,
		      const uint8_t mean_type)
{
#define MAX_DEPTH 32
	...
	while (1) {
		...
		if (recvupdate_cnt++ > recvupdatetime) {
			...
			if (make_mean(&pulse, tmp32, mean_nr, 1)) {
				if (pulse != pulse_old ||
				    timecnt >= msgupdatetime) {
					sprintf(print_buf,
						"Pulse = %04lu us   AVG = %02lu\n",
						pulse, mean_nr);
					print_usb(print_buf);
					pulse_old = pulse;
					timecnt = 0;
		...
		if (recvupdate_cnt++ > recvupdatetime) {
			...
			if (make_mean(&pulse, tmp32, mean_nr, 1)) {
				if (pulse != pulse_old ||
				    timecnt >= msgupdatetime) {
					sprintf(print_buf,
						"Pulse = %04lu   Angle = %03lu   AVG = %02lu\n",
						pulse, angle, mean_nr);
					print_usb(print_buf);
					pulse_old = pulse;
					timecnt = 0;
